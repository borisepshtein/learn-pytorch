"""Hair-excluded variant of scut_fbp_beauty_cross_race_transfer.py.

scut_fbp_beauty_classifier.py trained on Caucasian-female (CF) transferred poorly to
Asian-female (AF): ROC AUC dropped from 0.958 in-domain to 0.832 cross-domain, and the model's
confidence collapsed toward calling nearly everything "pretty" (mean predicted score 0.99 for
both classes). A follow-up ruled out sample size as the explanation (AF capped to CF's training
size still transferred fine in the other direction), and a simple hue/saturation/brightness
check on a crude hair-region band didn't show a strong within-population color effect either
-- but that check couldn't rule out the CNN exploiting hair color/texture in a way a regional
pixel average would miss. This script tests that more directly: crop every face to its facial
landmark bounding box (eyebrows to chin, cheek to cheek -- excludes hair almost entirely) using
the dataset's bundled 86-point landmarks, and rerun the same CF<->AF transfer test on those
crops. If cross-domain calibration/AUC improves once hair is excluded, hair was part of the
shortcut; if the gap persists, it isn't (or isn't primarily) about hair.
...
LANDMARK_MARGIN = 0.05  # fraction of bbox size added on each side, avoids clipping chin/cheek
...
LANDMARKS_DIR = os.path.join(EXTRACT_DIR, 'facial landmark')
...
def landmark_bbox(filename):
    """Bounding box of the 86 facial landmarks (eyebrows/eyes/nose/mouth/jawline), with a small
    margin -- excludes hair almost entirely since these landmark schemes stop at the eyebrows.
    Returns None if the landmark file is missing/corrupt (caller falls back to the full image)."""
    pts_path = os.path.join(LANDMARKS_DIR, os.path.splitext(filename)[0] + '.pts')
    try:
...
def face_crop(img, filename):
    bbox = landmark_bbox(filename)
    if bbox is None:
        return img
    w, h = img.size
...
for d, res in enumerate(direction_results):
    ...
    for col, idx in enumerate(list(top_idx) + list(bottom_idx)):
        ...
        img = face_crop(img, target_filenames[idx])
...
fig.savefig('results/scut_fbp_beauty_cross_race_transfer_no_hair.png', dpi=150)
print('\nSaved visualization to results/scut_fbp_beauty_cross_race_transfer_no_hair.png')
...
with open('results/scut_fbp_beauty_cross_race_transfer_no_hair_metrics.json', 'w') as f:
    json.dump(metrics, f, indent=2)
log_experiment('scut_fbp_beauty_cross_race_transfer_no_hair', metrics)
